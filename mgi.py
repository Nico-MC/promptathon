...
def get_auth_token(auth_url: str, username: str, password: str):
    """Holt den access_token vom Keycloak"""
...
def get_mgi_comments_by_go_nr(go_nr: str, auth_header):
...
def get_unvertraeglichkeiten_by_go_nr(go_nr: str, auth_header):
    res = requests.get(base_url + "/hinweise/unvertraeglichkeiten/zifferNr/" + go_nr, headers=auth_header)
    return res.json()


def get_bedingungen_by_go_nr(go_nr: str, auth_header):
    res = requests.get(base_url + "/hinweise/bedingungen/zifferNr/" + go_nr, headers=auth_header)
    return res.json()


def get_zusatznummern_by_go_nr(go_nr: str, auth_header):
    res = requests.get(base_url + "/hinweise/zusatznummern/zifferNr/" + go_nr, headers=auth_header)
    return res.json()


def get_originaernummern_by_go_nr(go_nr: str, auth_header):
    res = requests.get(base_url + "/hinweise/originaer/zifferNr/" + go_nr, headers=auth_header)
    return res.json()


def get_querverweise_by_go_nr(go_nr: str, auth_header):
    res = requests.get(base_url + "/hinweise/querverweise/zifferNr/" + go_nr, headers=auth_header)
    return res.json()


def get_complete_go_nr(go_nr: str, auth_header):
    """
    Bei Bedarf benötigte Bestandteile ein-/auskommentieren
    """
    ziffer = get_ziffer_by_go_nr(go_nr, auth_header)
    ziffer["kommentare"] = get_mgi_comments_by_go_nr(go_nr, auth_header)
    # ziffer["unvertraeglichkeiten"] = get_unvertraeglichkeiten_by_go_nr(go_nr, auth_header)
    # ziffer["zusatznummern"] = get_zusatznummern_by_go_nr(go_nr, auth_header)
    # ziffer["bedingungen"] = get_bedingungen_by_go_nr(go_nr, auth_header)
    # ziffer["originaernummern"] = get_originaernummern_by_go_nr(go_nr, auth_header)
    # ziffer["querverweise"] = get_querverweise_by_go_nr(go_nr, auth_header)

    return ziffer


def get_go_nrs_with_content(auth_header):
    # Liste der Ziffern, die geladen und gespeichert werden sollen
    # Kann auch mit beliebigen Ziffern befüllt werden
    # go_nrs = ["G"]
    go_nrs = get_go_nrs(auth_header)
    complete_ziffern = []

    log(f"Lade {len(go_nrs)} Ziffern...")
    n = 0
    for go_nr in go_nrs:
        complete_ziffern.append(get_complete_go_nr(go_nr, auth_header))
...
def main():
    ...
    auth_header = dict(Authorization="Bearer " + get_auth_token(auth_url, username, password))
    
    ziffern = get_go_nrs_with_content(auth_header)
